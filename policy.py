...
if __name__ == "__main__":
    # Load a pth file to TanhGaussianPolicy and test forward pass
    parser = argparse.ArgumentParser()
    parser.add_argument("--path", type=str, required=True, help="Path to the .pth file")
    ...
    policy.eval()
    sample_obs = torch.randn(4, obs_dim)
    action, mean, log_std, log_prob, std, pre_tanh_value = policy(
        sample_obs, deterministic=False, return_log_prob=True
    )
    print("Action:", action)
    print("Mean:", mean)
    ...
    print("Std:", std)
    print("Pre Tanh Value:", pre_tanh_value)
