...
class REDQSACAgent:
    def __init__(
        ...
        self.actor = GaussianPolicy(obs_dim, act_dim, hidden_sizes=(512, 512)).to(device)
    ...
    def select_action(self, obs: np.ndarray, deterministic: bool = False):
        """
        Handles both single observations (1D) and vectorized batches (2D).
        """
    ...
    def update(self, replay_buffer, synthetic_ratio=0.0):
        ...
        for _ in range(self.utd_ratio):
            self.update_steps += 1
            batch = replay_buffer.sample(self.batch_size, synthetic_ratio=synthetic_ratio)
            ...
            with torch.no_grad():
                ...
                target_q = rewards + self.gamma * (1 - dones) * (min_q_next - self.alpha * next_log_prob)
            ...
            # Average Q-value across ALL nets (or subset, REDQ paper suggests all for bias reduction)
            q_values_new = torch.cat([q_net(obs, new_actions) for q_net in self.q_nets], dim=1)
            mean_q_new = q_values_new.mean(dim=1, keepdim=True)

            actor_loss = (self.alpha * log_probs - mean_q_new).mean()
            
            if self.use_pgr and self.pgr_coef > 0.0 and self.update_steps % self.pgr_every == 0:
                ...
                for m in mu_pgr:
                    grads = torch.autograd.grad(
                        outputs=m, inputs=list(self.actor.parameters()),
                        retain_graph=True, create_graph=True
                    )
                    for g in grads:
                        pgr_loss += (g**2).sum()
                actor_loss += self.pgr_coef * pgr_loss
